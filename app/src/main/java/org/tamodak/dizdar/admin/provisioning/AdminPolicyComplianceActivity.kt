 ...
 * in a compliant state. Dizdar has nothing to enforce at this point — the user still has to set a
...
class AdminPolicyComplianceActivity : ComponentActivity() {
    ...
    /**
     * Shows the acknowledgement screen and returns `RESULT_OK` when the user continues.
     *
     * @param savedInstanceState standard Android instance state; unused, since the screen holds
     *   nothing worth restoring.
     */
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        DizdarLog.i(DizdarLog.ADMIN, "ADMIN_POLICY_COMPLIANCE shown — QR provisioning completed")
        // Matches MainActivity. Nothing secret is shown here, but every Dizdar window being
            ...
            DizdarTheme {
                Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
                    Column(
                        ...
                        Button(onClick = {
                            DizdarLog.d(DizdarLog.ADMIN) { "Compliance acknowledged; returning RESULT_OK" }
