 ...
 * @param onSubmit called on finger lift with the visited dot indices joined by `-`.
 * @param modifier applied to the grid.
 */
@Composable
fun PatternLock(
    enabled: Boolean,
    ...
    val dotColor = DizdarBorderMuted
    val activeColor = DizdarGreen
            ...
            .pointerInput(enabled) {
                ...
                detectDragGestures(
                    ...
                    onDragEnd = {
                        ...
                        selected.clear()
                        if (drawn.isNotEmpty()) {
                            DizdarLog.d(DizdarLog.UI) { "Pattern drawn: ${drawn.size} dots; submitting" }
                            onSubmit(drawn.toPatternString())
                        } else {
                            DizdarLog.v(DizdarLog.UI) { "Pattern gesture ended without touching a dot" }
                        }
                    },
                    onDragCancel = {
                        DizdarLog.v(DizdarLog.UI) { "Pattern gesture cancelled" }
                        fingerPosition = null
                        selected.clear()
                    },
                )
